...
#region Extensões 
        ...
        public static string ToLogin(this string nome)
        {
            nome = nome.RemoveAcentos();
            string ret = "";

            foreach (var item in nome.Split(' '))
                if (!new[] { "e", "de", "da", "das", "do", "dos" }.Contains(item))
                    ret += item.Substring(0, 1);
